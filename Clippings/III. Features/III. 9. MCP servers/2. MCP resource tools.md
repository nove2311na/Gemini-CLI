...
source: "https://geminicli.com/docs/tools/mcp-resources/"
author:
...
## 1\. list\_mcp\_resources (ListMcpResources)

`list_mcp_resources` retrieves a list of all available resources from connected MCP servers. This is primarily a discovery tool that helps the model understand what external data sources are available for reference.
...
- **Parameters:**
	- `serverName` (string, optional): An optional filter to list resources from a specific server.
- **Behavior:**
...
- **Output (`llmContent`):** A formatted list of available resources, including their URI, server name, and optional description.
- **Confirmation:** No. This is a read-only discovery tool.

## 2\. read\_mcp\_resource (ReadMcpResource)

`read_mcp_resource` retrieves the content of a specific resource identified by its URI.
...
- **Output (`llmContent`):** The content of the resource. For binary data, it returns a placeholder indicating the data type.
