...
Run Gemini CLI in headless mode by providing a prompt with the `-p` (or `--prompt`) flag. This bypasses the interactive chat interface and prints the response to standard output (stdout). Positional arguments without the flag default to interactive mode, unless the input or output is piped or redirected.
...
1. Save the following script as `generate_json.sh` (or `generate_json.ps1` for Windows):
...
You can add a function to your shell configuration to create a `git commit` wrapper that writes the message for you.
...
- Explore the [Headless mode reference](https://geminicli.com/docs/cli/headless) for full JSON schema details.
- Learn about [Shell commands](https://geminicli.com/docs/cli/tutorials/shell-commands) to let the agent run scripts instead of just writing them.